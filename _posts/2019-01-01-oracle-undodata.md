...
| 종류 | 내용 |
|:---:|:------------------------------------------------|
| **자동 언두 관리** | 자동 언두 관리는 오라클 커널에 의해 언두 세그먼트가 관리되고 조절된다. 단지 데이터베이스 관리자가 수행해야할 부분은 필요 시 언두 테이블스페이스를 증가시켜주는 것 밖에는 없다. 해당 언두 관리는 오라클 9i부터 가능한 관리 방식이다. 오라클 12c에서는 기본적으로 이 방식을 사용한다. |  
| **수동 언두 관리** |   |