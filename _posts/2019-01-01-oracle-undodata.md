...
| **Read Consistency** | 변경 중인 데이터 또는 변경된 데이터에 대한 조회 시 정합성 보장 |  
...
# 3. 언두 세그먼트의 개념
```
 언두 데이터를 저장하는 세그먼트 이다.
```

## 3.1 언두 세그먼트의 특징

![Alt text](/assets/images/undoextent.png "Oracle 12c")
...
### - 개념
```
1. 일반 세그먼트와 동일하게 확장할 수 있다.
2. 언두 세그먼트가 할당하고 있는 모든 익스텐트의 마지막 익스텐트까지 사용했다면 
   언두 세그먼트의 첫번째 익스텐트를 확인한다. 
   사용중이 아니라면 재사용을 하고
   사용중이라면 새로운 익스텐트를 할당하여 수행중인 작업들의 변경전 이미지를 저장한다.
   확장할 공간도 없으면 덮어 씌우게 된다.   
```
### - 상세
### - 요약

# 4. 언두 세그먼트의 종류 및 관리 방식
## 4.1 언두 세그먼트의 종류

![Alt text](/assets/images/undosegment.png "Oracle 12c")

| 종류 | 내용 |
|:---:|:------------------------------------------------|
| **시스템 언두 세그먼트** | 해당 언두 세그먼트는 시스템 테이블스페이스에 존재하는 오브젝트가 변경될 경우 사용되는 언두 세그먼트이다. 일반 유저의 변경 작업에는 사용되지 않는다. 시스템 언두 세그먼트는 데이터베이스 생성 시 시스템 테이블스페이스에 자동으로 생성된다. |  
| **비시스템 언두 세그먼트** | 해당 언두 세그먼트는 일반 유저의 DML 작업 시 언두 데이터를 저장하기 위해 사용되는 언두 세그먼트이다. |
| **지연 언두 세그먼트** | 특정 테이블스페이스가 오프라인, 임시 또는 For Recovery로 상태가 변경될 경우 진행중인 작업에 대해 복구를 수행하기 위해 해당 테이블스페이스에 대한 언두 세그먼트를 생성하여 언두 데이터를 보관한다. 이러한 용도로 생성되는 언두 세그먼트가 지연 언두 세그먼트이다.  |
...
| **언두 세그먼트** | 자동 관리 | 수동 관리 |
| **지연 언두 세그먼트** | 언두 테이블스페이스 증가 및 UNDO_RETENTION 파라미터 조정으로 에러 감소 | 언두 테이블스페이스 증가 및 언두 세그먼트 크기와 개수 조절 필요 |

| 종류 | 내용 |
|:---:|:------------------------------------------------|
| **AUtomatic Undo Management** | 자동 언두 관리는 오라클 커널에 의해 언두 세그먼트가 관리되고 조절된다. 단지 데이터베이스 관리자가 수행해야할 부분은 필요 시 언두 테이블스페이스를 증가시켜주는 것 밖에는 없다. 해당 언두 관리는 오라클 9i부터 가능한 관리 방식이다. 오라클 12c에서는 기본적으로 이 방식을 사용한다. |  
| **수동 언두 관리** |   |