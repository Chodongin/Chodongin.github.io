...
Row Source는 행을 반복적으로 처리할 수 있는 제어 구조와 함께 실행 계획의 단계별로 반환되는 행 집합이다. Row Source는 조인 또는 그룹화 작업의 테이블, 보기 또는 결과가 될 수 있다.

Row Source Generator는 Row Source Tree를 생성하며, 이 트리는 Row Source 집합이다. 행 원본 트리는 다음 정보를 보여준다.
